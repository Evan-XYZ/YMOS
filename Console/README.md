...
> 🖥️ **用桌面客户端的话**：Console（控制台首页 / 买卖决策台 / 交易计划台）与 Reader 由客户端**自带的内置服务器**直接提供，**不用手动 `python3 server.py`**、也不用配 `config.json`——绑定工作区即可用。这份说明面向命令行 / 自带 Agent 的直连方式；两种方式读写的是同一套 Markdown 真相源。
