...
YMOS 是自然语言驱动的人机协作投资系统。V4 = 三模块 + 运行时 + 决策台：

| 模块 | 职责 | 关键产出 |
|:---|:---|:---|
| **Eyes/** | 盯市场：市场洞察 + 投资雷达 | 市场日报 + 桥接报告 |
| **Brain/** | 做分析：策略路由 + 初始调研 + P系列提示词 + 投资诊断 | 策略报告 + 策略日志 + 诊断报告 |
| **持仓与关注/** | 管状态：状态机 + 个股文件夹 + 账户曲线 | 状态真相源 |
| **Agents/** | 定角色、定顺序、定边界（运行时编排） | 4 角色协议 + 调度链 |
| **Console/** | 把纪律变成物理闸门（人在用，不是 Agent 在用） | 交易计划 + 决策留痕 |
| **BrainStorm/** | 沉淀思考：Raw → Insight → 长期原则 | 感悟归档 + 长期记忆 |
...
### 关于 Agents/ 和 Console/ 的两条重要边界

- **`Agents/` 是给你（Agent）读的**：扮演某个角色前，先读 `Agents/EXECUTION_PLAYBOOK.md`
  里对应的五段式。核心纪律：**状态写回只有 Portfolio State Agent 一个出口**，
  其他角色再确信也只能"建议改"，不能自己动手。
- **`Console/` 不是给你用的**：它是人在浏览器里点的决策台。
  你可以**读** `Brain/交易计划/` 和 `Brain/决策审计/` 下的产出作为上下文
  （比如"用户昨晚的计划是什么""这周他被系统拦了几次"），
  但**不要替用户去写这两个目录**——那是他自己按下按钮才产生的记录，代写等于伪造审计。
...
| 持仓与关注 | SOP_周度复盘 | `跑一下周度复盘` | 每周 | Sequential |
| BrainStorm | SOP_头脑风暴 | `开始今天的头脑风暴` / `整理一下今天说的` | 每日 | Guided Loop |
| Brain | SOP_单标的P9P17专项调研 | `深研一下 XX` | 按需 | Sequential (Reusable) |
...
| `持仓与关注/SOP_周度复盘.md` | SOP 定义 |
| `Agents/*.md` | 运行时编排协议（角色、顺序、边界）|
| `Console/**` | 决策台前后端；**人在用，Agent 不改也不代填** |
| `Brain/交易计划/` | 决策台产出，**只读**——用户自己点出来的记录，代写等于伪造 |
| `Brain/决策审计/` | 决策留痕，**只读**，同上 |
| `BrainStorm/SOP_头脑风暴.md` | SOP 定义 |
| `总入口暗号.md` | 路由表 |
| `进阶指南.md` | 用户进阶文档 |
| `AGENT_GUIDE.md` | 本文件 |
| `.env` | API key |
...
| `BrainStorm/MEMORY.md` | 长期记忆 — **不得静默写**，必须用户确认 |
...
| `持仓与关注/账户资金曲线.md` | 用户口述当日盈亏时代记；周度复盘 SOP 追加周汇总 |
| `持仓与关注/击球区注册表.md` | P9/P17/P5 深研产生击球区时同步；不删行，失效改 status |
| `持仓与关注/主线首发信号跟踪.md` | 板块首发信号达标时 append / 刷新；退潮改 status |
| `BrainStorm/每日头脑风暴/Raw_Thoughts/YYYY-MM/` | 原始对话追加，不修饰 |
| `BrainStorm/Summarized_Insights/YYYY-MM/` | 结构化总结，整理时写入 |
| `BrainStorm/投资感悟归档/` | 感悟/框架/清单，用户确认后写 |
| `BrainStorm/状态机/STATUS.md` | 阶段状态，有变化时更新 |
...
5. **路由登记** → **同步 `总入口暗号.md`**（新能力必须在这里登记，否则等于没有入口）
6. **文档** → 更新 `进阶指南.md` + `README.md` + `CHANGELOG.md`

> 第 5 步最容易漏。暗号表是唯一的路由中枢，**没登记的能力，Agent 和用户都找不到。**
...
*Agent Guide 版本：2026-07-27 · YMOS V4 · 三模块 + 运行时 + 决策台*
