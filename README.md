...
3. **按需增加个人上下文** —— 市场范围、持仓、Watchlist 和关注方向都可以之后再补
...
| 维度 | 为什么重要 |
|:---|:---|
| **能力圈与市场范围** | 激活 Profile 后帮助投资雷达做个性化排序；未配置时仍输出市场与持仓/关注事实 |
...
| 投资雷达 | 市场洞察完成后 | 汇总市场、热门报告、持仓/关注、价格与事件；Profile 仅作可选排序增强 |
...
Profile 仍为 `draft` 时，市场洞察、投资雷达和事实调研照常运行：市场洞察无 API Key 会回退 RSS，投资雷达默认用 `balanced` 模式，也可切换 `price` 或 `event`。只有买卖、持有、加仓、退出和再平衡等动作级结论会被 `kernel_not_ready` 门禁拦住。
