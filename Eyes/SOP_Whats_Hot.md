...
**跨日累计怎么算**：读最近 3 份 `Whats_Hot_YYYY-MM-DD.md`（含今日三池）+ `持仓与关注/主线首发信号跟踪.md` 的 active 条目，按细分板块把去重后的入池个股累计计数；龙头 5 日涨幅取 `newhighs` 池 `涨跌幅[近5日]` 字段或单独补算。
...
- 写入/更新 `持仓与关注/主线首发信号跟踪.md`（新板块 append，已有板块累计成员并刷新阶段/龙头强度）。
...
> 触发即启动组级别 P17-lite（`Brain/references/p17-core-asset.md` §0.7），并写回 `持仓与关注/主线首发信号跟踪.md`。
...
8.5 `sis`（若触发）→ 写入报告 `## 🚨 板块首发信号` 段，并更新 `持仓与关注/主线首发信号跟踪.md`（新板块 append / 已有板块累计成员+刷新阶段）；作为 `SOP_Watchlist自动建档.md` 组级别 P17-lite 先手裁决的触发输入。
...
*版本：2026-07-01 · v2.5 · 新增 Step 4.0.1 板块首发信号（SIS）量化硬触发（近3日≥3只入池 + 龙头5日>30% + 明确催化）→ 输出 sis yaml + 报告🚨段 + 写回主线首发信号跟踪.md，next_action 升级为启动组级别 P17-lite（p17 §0.7）；把"多股指向同一板块"从提醒深研升级为先定代表权的先手裁决*
